...
namespace Day10
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            List<string> strechedGrid = new List<string>();
            strechedGrid.Add(new string('|', grid[0].Length));
            strechedGrid.Add(new string('|', grid[0].Length));
            foreach (string line in grid)
            {
                strechedGrid.Add(line);
                strechedGrid.Add(new string('|', line.Length));
            }


            grid = strechedGrid.ToArray();
            ...
            int[] currentPossition = start;
            int[] previousPossition = { 0, 0 };
            ...
            keyValuePairs.Add('S', 210);
            keyValuePairs.Add('|', 10);
            keyValuePairs.Add('-', 21);
            keyValuePairs.Add('L', 6);
            keyValuePairs.Add('F', 15);
            keyValuePairs.Add('7', 35);
            keyValuePairs.Add('J', 14);
            ...
            grid[start[1]] = grid[start[1]].Replace('S', 'L');


            do
            {
                int pipeValue = keyValuePairs[grid[currentPossition[1]][currentPossition[0]]];

                if (pipeValue % 2 == 0 && currentPossition[1] - 1 >= 0 && currentPossition[1] - 1 != previousPossition[1])
                {
                    path.Add(new int[] { currentPossition[0], currentPossition[1] - 1 });
                }
                else if (pipeValue % 3 == 0 && currentPossition[0] + 1 < grid[0].Length && currentPossition[0] + 1 != previousPossition[0])
                {
                    path.Add(new int[] { currentPossition[0] + 1, currentPossition[1] });
                }
                else if (pipeValue % 5 == 0 && currentPossition[1] + 1 < grid.Length && currentPossition[1] + 1 != previousPossition[1])
                {
                    path.Add(new int[] { currentPossition[0], currentPossition[1] + 1 });
                }
                else
                ...
                previousPossition = currentPossition;
                currentPossition = path.Last();

                //Console.Clear();
                //// Debug Print
                //for (int y = 0; y < strechedGrid.Count; y++)
                //{ // Y-Axis

                //    for (int x = 0; x < strechedGrid[0].Length; x++)
                //    { // X-Axis

                //        if (!ContainsPossition(path, new int[] { x, y }))
                //        {
                            
                            
                //                Console.Write(strechedGrid[y][x]);
            ...
            } while (path.Last()[0] != start[0] || path.Last()[1] != start[1]);
            ...
            List<int[]> frontLine = new List<int[]>();

            List<int[]> explered = new List<int[]>();
            explered.Add(new int[] { 0, 0 });
            List<int[]> allExplored = new List<int[]>();
            bool ContainsPossition(List<int[]> list, int[] possition)
            {
                bool willReturn = true;
                if (possition[0] == -1 || possition[1] == -1 || possition[1] >= strechedGrid.Count() || possition[0] >= strechedGrid[0].Length) { return true; }

                foreach (var item in list)
                {
                    if (item[0] == possition[0] && item[1] == possition[1]) { return true; }
                }
            ...
            while (exploring)
            {
                ...
                foreach (var f in explered)
                {
                    frontLine.Add(new int[] { f[0], f[1] });
                    allExplored.Add(new int[] { f[0], f[1] });
                    outside.Add(new int[] { f[0], f[1] });
                }
                if (frontLine.Count == 0) { break; }
                ...
                //        if (!ContainsPossition(path, new int[] { x, y }))
                //        {
                //            if (ContainsPossition(frontLine, new int[] { x, y }))
                //            {
                //            Console.Write('█');
                //            }
                //            else if (ContainsPossition(allExplored, new int[] { x, y }))
                //            {
                //                Console.Write('X');
                //            }
                //            else
                ...
                foreach (var possition in frontLine)
                {



                    int[] checkPoss = possition;

                    checkPoss[1]--;
                    try // UP
                    {

                        if (!ContainsPossition(allExplored, possition) && !ContainsPossition(explered, possition))
                        {
                            if (!ContainsPossition(outside, possition))
                            {
                                if (!ContainsPossition(path, possition))
                                {
                                    explered.Add(new int[] { checkPoss[0], checkPoss[1] });
            ...
            for (int y = 0; y < strechedGrid.Count; y += 2)
            { // Y-Axis

                for (int x = 0; x < strechedGrid[0].Length; x += 2)
                { // X-Axis


                    if (!ContainsPossition(outside, new int[] { x, y }))
                    {
                        if (!ContainsPossition(path, new int[] { x, y }))
                        {
                            Console.Write('█');
            ...
            // strech vert
            // find outside tiles
            // streched field - outside tiles - path tiles - tiles that contain oneven indexes
